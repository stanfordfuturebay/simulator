...
#     # Laura
#     if (essential_mob_rate_per_types is not None) and (essential_dur_mean_per_types is not None) and (self.essential_workers[indiv] is True):
...
def _simulate_individual_real_trace(indiv, max_time, site_type, mob_rate_per_type, dur_mean_per_type,
                               variety_per_type, delta, site_dist,verbose): # emma
    """Simulate a mobility trace for one real individual in a given town (jit for speed)"""
    # Holds tuples of (time_start, time_end, indiv, site, duration)
    data = list()
        
    # Set rates and probs
#     print('mob_rate_per_type',mob_rate_per_type,', dur_mean_per_type',dur_mean_per_type) # emma
    tot_mob_rate = np.sum(mob_rate_per_type)  # Total mobility rate
    site_type_prob = mob_rate_per_type / tot_mob_rate  # Site type probability
    # time
    t = rd.expovariate(tot_mob_rate)
    ...
    while t < max_time:        
        ...
        if len(usual_sites[k]) != 0:
            site = np.random.choice(np.array(usual_sites[k]))         
        # Duration: Exponential
#         print('k',k,', site',site,' , dur_mean',dur_mean_per_type)
        dur = rd.expovariate(1/dur_mean_per_type[k])        
        if t + dur > max_time:
            break
        # Add visit namedtuple to list
...
def _simulate_synthetic_mobility_traces(*, num_people, num_sites, max_time, home_loc, site_loc,
                            ...
                            delta, seed, essential_workers, essential_mob_rate_per_type, essential_dur_mean_per_type):
    rd.seed(seed)
...
def _simulate_real_mobility_traces(*, num_people, max_time, site_type, people_age, mob_rate_per_age_per_type,
                            dur_mean_per_type, home_tile, tile_site_dist, variety_per_type, delta, seed,verbose, essential_workers, essential_mob_rate_per_type, essential_dur_mean_per_type): # emma
    rd.seed(seed)
    np.random.seed(seed-1)
    data, visit_counts = list(), list()       
...
class MobilitySimulator:
    ...
    def __init__(self, delta, home_loc=None, people_age=None, site_loc=None, site_type=None,
                ...
                essential_workers=None, essential_mob_rate_per_type=None, essential_dur_mean_per_type=None):#Laura Change, emma
        """
        delta : float
        ...
        real = (home_loc is not None and people_age is not None and site_loc is not None and site_type is not None and
                ...
                tile_site_dist is not None and variety_per_type is not None and downsample is not None and
                essential_workers is not None and essential_mob_rate_per_type is not None and essential_dur_mean_per_type is not None)#emma
...
#             print('a',self.essential_mob_rate_per_type) # emma
...
#             print(self.essential_mob_rate_per_type) # emma
